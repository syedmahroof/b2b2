...
class SupplierProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('featured_image')) {
            ...
            $slider = $request->file('featured_image');

            // Generate a unique filename for the logo
            $sliderFileName = 'featured_image_' . rand(10000, 99999) . time() . '.' . $slider->getClientOriginalExtension();

            // Store the logo in a designated directory (e.g., storage/app/public/logos)
            $slider->storeAs('public/featured_images', $sliderFileName);

            $data['featured_image'] = 'featured_images/'.$sliderFileName;

...
class VariantSelects extends HTMLElement {
  ...
  updateVariantInput() {
    ...
    productForms.forEach((productForm) => {
      ...
      if(window.WhatSub) {
        window.WhatSub.dispatch('UPDATE_VARIANT', this.currentVariant);
      }
    });
  }
  ...
  renderProductInfo() {
    fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&section_id=${this.dataset.originalSection ? this.dataset.originalSection : this.dataset.section}`)
      ...
      .then((responseText) => {
        ...
        // Fix price for personalised products
        const destinationPricePP = document.querySelector(`.m-pp-view__price`);
        const sourcePricePP = html.querySelector(`.m-pp-view__price`);
        if (sourcePricePP && destinationPricePP) destinationPricePP.innerHTML = sourcePricePP.innerHTML;
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    const mainProduct = document.querySelector('.o-main-product')
    ...
    if (disable) {
      addButton.setAttribute('disabled', 'disabled');
        ...
        if (mainProduct.classList.contains('status-sale-ended')) {
          if (text){
            addButtonText.textContent = text;
          }
        }
        ...
        if(addButton.classList.contains('a-btn--pre-order')){

...
Line 64:12 Transforming const to the configured target environment ("es5") is not supported yet
Line 74:10 Transforming const to the configured target environment ("es5") is not supported yet
Line 94:22 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
...
if (!customElements.get('product-form')) {
  customElements.define('product-form', class ProductForm extends HTMLElement {
    ...
    onSubmitHandler(evt) {
      ...
      // return when personalisation or badge input is not empty so we can handle the submit in PersonalisationProductDetailTool.js
      if(this.submitButton.classList.contains('is-personalised-product')) {
        const propertyPrint = this.form.querySelector('.field_personalisation');
        const propertyBadge = this.form.querySelector('.field_badge');

        if(propertyPrint.value !== '' || propertyBadge.value !== '') {
          return
        }
      }
